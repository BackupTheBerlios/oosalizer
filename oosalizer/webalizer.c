...
int     ntop_notfound= 50;                    /* top n 404 to display     */
      ...
      if (parse_record(buffer))           /* parse the record             */
      {
		...
		if (cp1!=NULL)
		{
         ...
         if (cur_month != rec_month)
         {
            ...
            cur_year  = rec_year;
            f_day=l_day=rec_day;
	    free(respnotfound);      /* free old memory from the last month */
	    /* Alloc memory for the 404 errors with MAXRESP size */
	    void * tmp_ptr = calloc(MAXRESP, sizeof(struct response_url));
	    if (tmp_ptr) {
		respnotfound = (struct response_url *) tmp_ptr;
	    ...
	    maxresp      = MAXRESP;     /* reset constant for realloc    */
...
void get_config(char *fname)
{
   char *kwords[]= { "Undefined",         /* 0 = undefined keyword       0  */
                     ...
                     "CodeNotFound"       /* Top 404 errors             88  */
                   };
   ...
   while ( (fgets(buffer,BUFSIZE,fp)) != NULL)
   {
      ...
      *cp2--='\0';
      ...
      switch (key)
      {
        ...
        case 88: ntop_notfound = atoi(value);     break;  /* Top 404 errors */
      }
   }
   fclose(fp);
}
